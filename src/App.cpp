...
App::App() : hspi(HSPI), sht4(), display(GxEPD2_DRIVER_CLASS(Config::Pins::EPD_CS, Config::Pins::EPD_DC, Config::Pins::EPD_RES, Config::Pins::EPD_BUSY))
{
...
void App::initHardware()
{
  ...
  //display = new GxEPD2_DISPLAY_CLASS<GxEPD2_DRIVER_CLASS, MAX_HEIGHT(GxEPD2_DRIVER_CLASS)>(GxEPD2_DRIVER_CLASS(Config::Pins::EPD_CS, Config::Pins::EPD_DC, Config::Pins::EPD_RES, Config::Pins::EPD_BUSY));


    display.init(115200, true, 10, 1, hspi, SPISettings(4000000, MSBFIRST, SPI_MODE0));
    //display.setPartialWindow(0,0,800,256);
...
void App::enterSleep()
{
  ...
  digitalWrite(Config::Pins::SD_DET, LOW);
  digitalWrite(Config::Pins::SD_EN, LOW);
